...
// Start the gap stuff for BT Classic
static esp_err_t init_bt_gap(void)
{
	esp_err_t 					ret = ESP_FAIL;
	static const char* 			TAG = "init_bt_gap";

	ESP_LOGI(TAG, "init_bt_gap");
	...
	// Start the callback 
	ESP_GOTO_ON_ERROR( esp_bt_gap_register_callback(bt_gap_event_handler), 						error, TAG, "esp_bt_gap_register_callback failed" );

	ESP_GOTO_ON_ERROR( esp_bt_gap_set_scan_mode(ESP_BT_CONNECTABLE, ESP_BT_NON_DISCOVERABLE), 	error, TAG, "esp_bt_gap_set_scan_mode failed" );

    return ESP_OK;

error:
	// Send error message
	ESP_LOGE( TAG, "init_bt_gap failed: ( %d )", ret );
...
{
    esp_err_t 					ret = ESP_OK;	// TODO: SET TO FAIL
	static const char* 			TAG = "init_bt_gap";

	ESP_LOGI(TAG, "start_bt_scan");

	ESP_GOTO_ON_ERROR( 	esp_bt_gap_start_discovery(ESP_BT_INQ_MODE_GENERAL_INQUIRY, (int)(seconds / 1.28), 0)
						, error, TAG, "esp_ble_gap_register_callback failed" );

    return ESP_OK;

error:
	// Send error message
	ESP_LOGE( TAG, "start_bt_scan failed: ( %d )", ret );
...
// Start the gap stuff for BLE
static esp_err_t init_ble_gap(void)
{
	esp_err_t 					ret = ESP_FAIL;
	static const char* 			TAG = "init_ble_gap";

	ESP_LOGI(TAG, "init_ble_gap");

	ESP_GOTO_ON_ERROR( esp_ble_gap_register_callback(ble_gap_event_handler), error, TAG, "esp_ble_gap_register_callback failed" );

    return ESP_OK;

error:
	// Send error message
	ESP_LOGE( TAG, "init_ble_gap failed: ( %d )", ret );
...
esp_err_t esp_hid_ble_gap_adv_init(uint16_t appearance, const char *device_name)
{
    esp_err_t ret;
	//esp_err_t 		ret = ESP_FAIL;
	static const char* 	TAG = "esp_hid_ble_gap_adv_init";

	ESP_LOGI(TAG, "esp_hid_ble_gap_adv_init");
...
static esp_err_t init_low_level(uint8_t mode)
{
	...
	static const char* 			TAG = "init_low_level";

	ESP_LOGI(TAG, "init_low_level");
	
	#if CONFIG_IDF_TARGET_ESP32
		bt_cfg.mode = mode;		// Set to dual mode
	#endif
	bt_cfg.bt_max_acl_conn = 3;
	bt_cfg.bt_max_sync_conn = 3;

	// Init bt controller
	ESP_GOTO_ON_ERROR( esp_bt_controller_init(&bt_cfg),	error, TAG, "esp_bt_controller_init failed" );
	ESP_GOTO_ON_ERROR( esp_bt_controller_enable(mode), 	error, TAG, "esp_bt_controller_enable failed" );

	// Init bluedroid?!?!
	ESP_GOTO_ON_ERROR( esp_bluedroid_init(), 	error, TAG, "esp_bluedroid_init failed" );
	ESP_GOTO_ON_ERROR( esp_bluedroid_enable(), 	error, TAG, "esp_bluedroid_enable failed" );

	// Init the gap for bt and ble
	ESP_GOTO_ON_ERROR( init_bt_gap(), 	error, TAG, "init_bt_gap failed" );
	ESP_GOTO_ON_ERROR( init_ble_gap(), 	error, TAG, "init_ble_gap failed" );

    return ESP_OK;

error:
	// Send error message
	ESP_LOGE( TAG, "init_low_level failed: ( %d )", ret );
...
{
	esp_err_t 			ret = ESP_FAIL;
	static const char* 	TAG = "esp_hid_gap_init";

	ESP_LOGI(TAG, "esp_hid_gap_init");

	// Prepare for Bluetooth Classic
    bt_hidh_cb_semaphore = xSemaphoreCreateBinary();
	ESP_GOTO_ON_FALSE( ( bt_hidh_cb_semaphore != NULL ), ESP_FAIL, error, TAG, "bt_hidh_cb_semaphore xSemaphoreCreateMutex failed" );

	// Prepare for BLE
    ble_hidh_cb_semaphore = xSemaphoreCreateBinary();
	ESP_GOTO_ON_FALSE( ( bt_hidh_cb_semaphore != NULL ), ESP_FAIL, error, TAG, "ble_hidh_cb_semaphore xSemaphoreCreateMutex failed" );

	// Init low level
	ESP_GOTO_ON_ERROR( init_low_level(mode), error, TAG, "init_low_level failed" );
...
{
	esp_err_t 			ret = ESP_FAIL;
	static const char* 	TAG = "esp_hid_scan";

	ESP_LOGI(TAG, "esp_hid_scan");
